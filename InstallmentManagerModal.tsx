    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4" aria-modal="true" role="dialog">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="p-6 border-b grid grid-cols-3 gap-4 text-center">
            ...
            <div><p className="text-sm text-slate-500">Valor Pago</p><p className="font-bold text-lg text-green-600">R$ {paidAmount.toFixed(2)}</p></div>
            ...
            {fee.installments.map((inst, index) => (
              <li key={inst.id} className="flex items-center justify-between p-3 rounded-lg bg-slate-50">
                <div>
                  ...
                  <p className="text-sm text-slate-500">Vencimento: {new Date(inst.dueDate).toLocaleDateString('pt-BR')}</p>
                </div>
                <div className="flex items-center">
                  <span className={`mr-4 text-sm font-semibold ${inst.status === 'Pago' ? 'text-green-600' : 'text-yellow-600'}`}>{inst.status}</span>
                  <button 
                    onClick={() => handleStatusChange(inst.id, inst.status === 'Pendente' ? 'Pago' : 'Pendente')}
                    className={`p-2 rounded-full transition-colors ${inst.status === 'Pago' ? 'bg-green-500 text-white hover:bg-green-600' : 'bg-slate-200 text-slate-600 hover:bg-slate-300'}`}
                  >
        ...
        <div className="p-6 bg-slate-100 border-t flex justify-end">
          <button onClick={onClose} className="px-4 py-2 bg-blue-600 border border-transparent rounded-md shadow-sm text-sm font-medium text-white hover:bg-blue-700">Fechar</button>
        </div>
      </div>
    </div>
  );
