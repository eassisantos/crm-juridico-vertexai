    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4" aria-modal="true" role="dialog">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          <p className="text-slate-600 mb-4">Você tem {urgentTasks.length} tarefa(s) vencendo nos próximos 7 dias ou em atraso.</p>
            ...
            {urgentTasks.map(task => {
              ...
              return (
                <li key={task.id} className="py-4 flex items-center justify-between hover:bg-slate-50 -mx-6 px-6">
                  <Link to={`/cases/${task.caseId}`} onClick={() => onClose(false)} className="flex-1 min-w-0">
                    <p className={`font-medium ${isOverdue ? 'text-red-700' : 'text-blue-600'} truncate`}>{task.description}</p>
        ...
        <div className="p-6 bg-slate-50 border-t flex justify-end space-x-3">
          ...
          <button onClick={() => onClose(false)} className="px-4 py-2 bg-blue-600 border border-transparent rounded-md shadow-sm text-sm font-medium text-white hover:bg-blue-700">
            Fechar
          </button>
        </div>
      </div>
